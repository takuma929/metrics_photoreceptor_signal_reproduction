function plot_chromaticity_alpha(varargin)
...
if isempty(parentAxis)
    ...
    plot(locus(:,1),locus(:,2),'k','LineWidth',plotLineWidth,'LineStyle','none');
else
    plot(parentAxis, locus(:,1),locus(:,2),'k','LineWidth',plotLineWidth,'LineStyle','none');
end
...
plot(plotAxis, [locus(1,1) locus(end,1)], [locus(1,2) locus(end,2)],'k','LineWidth',plotLineWidth,'LineStyle','none')
...
for i = 1:4:rows
    ...
    patch('Vertices',v, 'Faces',f, 'EdgeColor','none', ...
        'FaceVertexCData',xy4rgb(i:i+3,3:5),'FaceColor','interp','FaceAlpha',0.3)
end
...
if nargin > 0 && nargin ~= 2
    ...
    for j=1:numColorPatches
        ...
        dp = p2-p1;
        quiver(plotAxis, p1(1),p1(2),dp(1),dp(2),0,'k','LineWidth',1, 'MaxHeadSize',0.9);hold on;
    end
    if displayROIIndex
